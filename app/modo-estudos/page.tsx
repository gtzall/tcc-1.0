...
export default function StudyModePage() {
  ...
  const startStudySession = () => {
    ...
    const diff = studySession.difficulty === 'easy' ? 'facil' : studySession.difficulty === 'medium' ? 'medio' : 'dificil'
    const subject = selectedSubjects[0] || 'geral'
    const config = {
      subject,
      difficulty: diff,
      institution: 'geral',
      questionCount: studySession.questionCount,
      timeLimit: studySession.timeLimit * 60
    }
    setQuizConfig(config)
    setGameState('playing')
  }

  if (gameState === 'playing' && quizConfig) {
    return (
      <QuizGame
        config={quizConfig}
        onFinish={(results) => { setQuizResults(results); setGameState('results') }}
        onBack={() => { setGameState('setup'); setQuizConfig(null); setQuizResults(null) }}
        useApi
      />
    )
  }
